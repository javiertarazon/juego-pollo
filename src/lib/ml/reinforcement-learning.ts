...
export async function loadMLState() {
  try {
    ...
    // Calcular tasas de éxito por posición
    games.forEach((game) => {
      ...
      if (revealed.length > 0) {
        ...
        // Actualizar Q-value basado en éxito histórico
        const successRate =
          mlState.positionSuccessRate[firstPos].wins /
          mlState.positionSuccessRate[firstPos].total;
    ...
    // Obtener últimas 7 posiciones seguras usadas
    ...
    // Degradar epsilon basado en total de partidas
    mlState.epsilon = Math.max(
      MIN_EPSILON,
      0.3 * Math.pow(EPSILON_DECAY, mlState.totalGames)
    );

    console.log(`ML State cargado: ${mlState.totalGames} partidas, epsilon: ${mlState.epsilon.toFixed(3)}`);
  } catch (error) {
